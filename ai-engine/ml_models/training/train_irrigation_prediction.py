...
Model   : Irrigation Prediction (RandomForestClassifier)
Dataset : data/cleaned/irrigation_prediction_cleaned.csv

This script ONLY trains and evaluates the model. No Django/API code here.

Input   : data/cleaned/irrigation_prediction_cleaned.csv
Outputs : ml_models/artifacts/irrigation_model.pkl               (full sklearn Pipeline:
                                                                    preprocessing + RandomForest)
          ml_models/artifacts/irrigation_feature_preprocessor.pkl (ColumnTransformer only,
                                                                    saved separately for inspection/reuse)
          ml_models/artifacts/irrigation_label_encoder.pkl        (target LabelEncoder)
"""
...
# ── Config ───────────────────────────────────────────────────────────
DATA_PATH = os.path.join("data", "cleaned", "irrigation_prediction_cleaned.csv")
ARTIFACTS_DIR = os.path.join("ml_models", "artifacts")
MODEL_PATH = os.path.join(ARTIFACTS_DIR, "irrigation_model.pkl")
PREPROCESSOR_PATH = os.path.join(ARTIFACTS_DIR, "irrigation_feature_preprocessor.pkl")
LABEL_ENCODER_PATH = os.path.join(ARTIFACTS_DIR, "irrigation_label_encoder.pkl")
...
def load_data(path: str) -> pd.DataFrame:
    """Load the cleaned irrigation prediction dataset."""
    df = pd.read_csv(path)
...
def build_feature_preprocessor() -> ColumnTransformer:
    ...
    (Soil_Type, Crop_Type, Water_Source, etc. have no inherent order),
    which is the correct choice for RandomForest to avoid implying a
    false ranking between categories.
...
def encode_target(y):
    """
    Encode the target 'Irrigation_Need' labels (Low/Medium/High) into
    integers using LabelEncoder. Returns encoded labels and the fitted
    encoder (needed later for inverse_transform when serving predictions).
    """
    label_encoder = LabelEncoder()
    y_encoded = label_encoder.fit_transform(y)
    print(f"[ENCODE] Irrigation_Need classes: {list(label_encoder.classes_)}")
...
def split_train_test(X, y_encoded):
    ...
    to preserve class balance across Low/Medium/High irrigation need.
    """
...
def evaluate_model(pipeline: Pipeline, X_test, y_test, label_encoder: LabelEncoder) -> None:
    ...
    print("\n" + "=" * 60)
    print("MODEL EVALUATION — Irrigation Prediction")
    print("=" * 60)
...
def save_artifacts(pipeline: Pipeline, label_encoder: LabelEncoder) -> None:
    """
    ...
    - The target LabelEncoder, saved separately, to decode predicted class
      indices back into Low/Medium/High irrigation need labels.
    """
